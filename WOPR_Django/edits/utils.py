...
def makeChoicesList_EditsQualityCheck():
    ...
    with connection.cursor() as cursor:
        cursor.execute('SELECT DISTINCT [siteid],[Description] FROM [t_sites]')
        siteNames = dictfetchall(cursor)
        siteList = []
...
def makeSiteList():
    ...
    with connection.cursor() as cursor:
        ...
        siteNames = dictfetchall(cursor)
        siteList = []
        siteList.append((' ', ' ')) #need a blank one so we know when to populate the turbine list
