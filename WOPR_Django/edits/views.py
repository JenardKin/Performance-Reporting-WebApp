...
def selectSite(request):
    ...
    if request.method == 'GET':
        ...
        return render(request, 'selectSite.html', {'form': form, 'title': 'Select a Site'})
    ...
    if request.method == 'POST':
        ...
        return render(request, 'selectSite.html', {'form': form,  'title': 'Select a Site', 'message': 'Please make sure to enter a valid site id.'})
...
def getEvents(request, site):
    ...
    return render(request, 'eventsData.html', {'json': dataFromTemplate})
...
def getEnergy(request, site):
    ...
    return render(request, 'energyData.html', {'json': dataFromTemplate})
...
def getSiteConfig(request, site):
    ...
    # with a Content-Type header set to application/json 
    return render(request, 'siteconfig.html', {'json': dataFromTemplate})    
...
def getEdits(request, site):
    ...
    # Send back the JSON containing the data of t_sites to the AJAX call
    # with a Content-Type header set to application/json 
    return render(request, 'editsTable.html', {'json': dataFromTemplate})
...
def dashboardSelectSite(request):
    ...
    if request.method == 'GET':
        ...
        return render(request, 'dashboard.html', {'siteSelectionForm': siteSelectionForm, 'title': 'Edits Allocation'})
...
def dashboard(request, site):
    ...
    if request.method == 'GET':
        ...
        if('reselect' in request.GET):
            ...
            if(form.CHOICES[0][0] == -1 and form.CHOICES[0][1] == 'There are no turbines available for this site.'):
                ...
                return render(request, 'selectSite.html', {'message': 'There were no turbines in the database for that site.', 'title': 'Select a Site', 'form': siteform})
        ...
        # when the user has filled out the filterdashboard form
            ...
            return render(request, 'dashboard.html', {'form': form, 'ts_start_global': request.session.get('ts_start_global'), 'ts_end_global': request.session.get('ts_end_global'), 'turbine_range': request.session.get('turbine_range'),
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Return the dictionary with the list of turbine information            
            return render(request, 'dashboard.html', {'form': form, 'ts_start_global': ts_start_global.__str__, 'ts_end_global': ts_end_global.__str__, 'turbine_range': turbine_range, 'filterDashboardForm': filterDashboardForm,
            ...
            return render(request, 'dashboard.html', {'form': form, 'filterDashboardForm': filterDashboardForm})
...
def editsTableView (request):
    template_name = 'editsTable2.html'
...
def commitPage(request):
    ...
    return render(request, 'commitPage.html', {'title': 'Commit Page', 'ts_start_global': request.session.get('ts_start_global'), 'ts_end_global': request.session.get('ts_end_global'), 'turbine_range': request.session.get('turbine_range'),
     'stateColorScheme': json.dumps(stateColorScheme, cls=DjangoJSONEncoder), 'systemColorScheme': json.dumps(systemColorScheme, cls=DjangoJSONEncoder)})
...
def getEditsReport(request):
    ...
    return render(request, 'reports/editsReport.html', {'quality_report':unorderedDict})

def QualityCheckView(request):
    template_name = 'reports/editsQualityCheckReport.html'
...
def top20FaultsView(request):
    template_name = 'reports/top20FaultsReport.html'
...
def recurringFaultsView(request):
    ...
    template_name = 'reports/recurringFaultReport.html'
    # blank form
    if request.method == 'GET':
        form = SiteTimeRangeForm()
        return render(request, template_name, {'form':form})
...
def powerCurveView(request):
    template_name = 'reports/powerCurveReport.html'
    # blank form
...
def filterTurbines(request):
    if (request.is_ajax and request.GET):
        ...
        return render(request, 'formtemplates/turbine_dropdown_options.html', {'turbines': turbines})

    return render(request, 'formtemplates/turbine_dropdown_options.html', {'turbines': {}})

def powerCurveCompareView(request):
    template_name = 'reports/powerCurveComparisonReport.html'
...
#     template_name = 'reports/siteStartTimeEndTimeForm.html'
...
#         form = siteID_startTime_endTime()
#         return render(request, self.template_name, {'form':form})
#     return render(request, 'reports/siteStartTimeEndTimeForm.html', {'form':form})
